...
def train_glove_model(corpus):
    glove_model = gensim.models.KeyedVectors.load_word2vec_format(build_gensim_glove_file(),binary=False)
    #model.save(W2V_MODEL_FILE)
    print("========= EMBEDDING SAVED =========")
    print("\tglove model file : {}".format(GLOVE_MODEL_FILE))
    print("\tvocab size : {}".format(len(glove_model.wv.vocab)))
    return glove_model

def load_glove_model_file(filename):
    print("========= LOADING GLOVE PRETRAINED MODEL =========")
    glove_model = gensim.models.KeyedVectors.load_word2vec_format(GLOVE_MODEL_FILE,binary=False)
    print("\tvocab size : {}".format(len(glove_model.wv.vocab)))
    return glove_model

def train_w2v_model(corpus):
    w2v_model = gensim.models.Word2Vec(sentences=corpus, size=N_FEATURES,
...
def create_review_vectors(corpus, w2v_model):
    ...
    for review in corpus:
        feature_vec[idx] = vec_from_review(review, w2v_model)
        idx = idx + 1
...
if __name__ == "__main__":
    ...
    if args.glove_trained:
        glove_model = load_glove_model_file(GLOVE_MODEL_FILE)
    else:
        glove_model = train_glove_model(train_corpus)

    if args.w2v_trained:
        w2v_model = load_w2v_model_file(GLOVE_MODEL_FILE)
    else:
        w2v_model = train_w2v_model(train_corpus)

    w2v_train_vec, w2v_test_vec = get_vectorized_data(train_corpus, test_corpus, w2v_model)
    glove_train_vec, glove_test_vec = get_vectorized_data(train_corpus, test_corpus, glove_model)

    print("============ Word2Vec Model ============")
    exec_logistic_regression(w2v_train_vec, w2v_test_vec, data)
    exec_deep_neural_net(w2v_train_vec, w2v_test_vec, data)

    print("============= GloVe Model ==============")
    exec_logistic_regression(glove_train_vec, glove_test_vec, data)
    exec_deep_neural_net(glove_train_vec, glove_test_vec, data)
